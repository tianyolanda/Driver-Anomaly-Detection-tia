...
class Conv3dStaticSamePadding(nn.Conv3d):
    ...
    def forward(self, x):
        ...
        print(x.shape)
        print('[self.weight, self.bias, self.stride, self.padding, self.dilation, self.groups]',self.weight.shape, self.bias, self.stride, self.padding, self.dilation, self.groups)
...
def get_model_params(model_name, override_params):
    ...
    if model_name.startswith('efficientnet'):
        ...
        print('w, d, s, p',w, d, s, p)
        for i in range(len(blocks_args)):
            print(blocks_args[i])
        print(global_params)
    else:
